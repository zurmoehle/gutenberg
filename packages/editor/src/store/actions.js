...
export const setupEditor = ( post, edits, template ) => ( { dispatch } ) => {
	...
	dispatch( { type: 'SETUP_EDITOR', post, edits, template } );
...
export const savePost = ( options = {} ) => async ( {
	...
	let edits = {
		content: select.getEditedPostContent(),
	};

	if ( ! options.isAutosave ) {
		dispatch.editPost( edits, { undoIgnore: true } );
	}

	dispatch.__experimentalRequestPostUpdateStart( options );
	...
	dispatch.__experimentalRequestPostUpdateFinish( options );
 ...
 */
export const refreshPost = () => async ( { select, dispatch, registry } ) => {
